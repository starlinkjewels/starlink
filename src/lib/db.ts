...
export type ManufacturingLogEntryType =
  "factory_assigned" | "gold_issued" | "piece_finished" | "making_charge_added";
...
export interface Order {
  ...
  goldIssuanceIds?: string[]; // GoldIssuance docs for this order — each carries its own factoryId,
                              // so this alone already supports >1 factory per order
...
export interface Locker {
  ...
  openingBalance: number; // INR — one-time scalar, like Client.creditBalance
  createdAt: string;
...
export interface LockerTransaction {
  ...
  lockerId: string;
  type: LockerTxnType;
  amountInr: number; // always positive; sign implied by `type`
  category?: string; // "Supplier Payment" | "Factory Making Charges" | "Owner Deposit" | "Local Expense" | ...
  refType?: "purchase" | "goldIssuance" | "manual" | "transfer";
  refId?: string; // Purchase.id / GoldIssuance.id, when auto-generated by a payment entry
...
export interface FinishedPiece {
  id: string;
  weightUsedGrams: number; // actual gold weight that ended up in the finished piece(s) from this issuance
  piecesCount?: number;
  recordedAt: string;
  recordedBy: string;
  notes?: string;
...
export interface StockMovement {
  ...
  refType?: "purchase" | "goldIssuance" | "order" | "manual";
  refId?: string;
  createdBy: string;
  createdAt: string;
  note?: string;
