...
export function LoginPage() {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden"
      style={{ background: "radial-gradient(circle at 20% 20%, oklch(0.50 0.11 48) 0%, oklch(0.33 0.07 44) 60%, oklch(0.22 0.05 42) 100%)" }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative w-full max-w-md">
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-luxe p-8 md:p-10">
          <div className="flex flex-col items-center text-center mb-8">
            <img src="/starlink-logo.png" alt="Flenix Jewels" className="h-20 w-auto object-contain mb-6" />
            <h1 className="font-display text-3xl text-brand-dark">Welcome Back</h1>
            <p className="text-sm text-muted-foreground mt-1">Sign in to the Flenix Jewels client portal</p>
          </div>
        ...
        <p className="text-center text-white/60 text-xs mt-6">© {new Date().getFullYear()} Flenix Jewels — Fine Diamond Jewelry, USA</p>
      </motion.div>
    </div>
  );
}